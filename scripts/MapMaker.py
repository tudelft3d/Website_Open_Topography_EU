...
def special_match_mask(layer_gdf, name):
    ...
    for column in candidate_columns:
        ...
        mask = (
            ...
            | values.eq(name)
            | values.str.contains(
                rf"(?:^|[|,;])\s*{re.escape(name)}\s*(?:$|[|,;])", regex=True
            )
        )
        ...
        if gadm_gpkg and os.path.exists(gadm_gpkg):
            try:
                ...
                if special_mask.any():
                    ...
                    col in special_gdf.columns
                    for col in ["place", "Place", "PLACE", "name", "Name", "NAME"]
                ):
                    logger.warning(
                ...
                for _, feat in special_gdf.iterrows():
                    ...
                    for col in getattr(special_gdf, "columns", []):
                        ...
                        if feat_value is None or (
                            hasattr(pd, "isna") and pd.isna(feat_value)
                        ):
                            continue
                        if (
                            col not in matched_row
                            or matched_row[col] is None
                            or (
                                isinstance(matched_row[col], str)
                                and not str(matched_row[col]).strip()
                            )
                        ):
            ...
            logger.warning(
                f"No match found for {lookup_name} and no gpkg in {special_dir}"
            )
            return matched_rows
    else:
        # Normal match
...
def match_names_and_export(gadm_gpkg, input_file, output_dir, special_dir):
    """
...
@click.option(
    "--output-dir", type=click.Path(), help="Output directory for GeoJSON files"
)
@click.option(
    "--special-dir",
    type=click.Path(exists=True),
